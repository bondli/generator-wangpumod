...
//如果`<%= className %>`被修改成别的，请将下面的`<%= className %>`也对应修改
...
/*
 * 如下部分不要修改，（除非你前面的`<%= className %>`被修改了，那就修改下面的`<%= className %>`为你的最终值）否则模块加载不出来！
 */
window.ShopRender.<%= className %> = function(container, data){
  var moduleData = JSON.parse(data);
  ReactDOM.render(
    <<%= className %> moduleData={moduleData} />, container
  );
};
